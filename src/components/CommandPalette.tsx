...
export default function CommandPalette({
  ...
  const items: Item[] = [
    { label: "Work", hint: "Featured project", icon: <FolderGit2 size={15} />, action: () => go("#work") },
    { label: "Engineering Lab", hint: "Smaller builds", icon: <FlaskConical size={15} />, action: () => go("#lab") },
    { label: "About", hint: "Background & timeline", icon: <User size={15} />, action: () => go("#about") },
    { label: "Resume", hint: "Open PDF", icon: <FileText size={15} />, action: () => window.open("/Jerin_Johnson_Resume.pdf", "_blank") },
    { label: "GitHub", hint: "View profile", icon: <GithubIcon size={15} />, action: () => window.open("https://github.com", "_blank") },
    { label: "Contact", hint: "Send a message", icon: <Mail size={15} />, action: () => go("#contact") },
  ];
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 px-4 py-3 border-b border-[var(--color-border)]">
          <span className="font-mono text-[var(--color-accent)] text-sm">{">"}</span>
          <input
            autoFocus
            value={query}
        ...
        <div className="max-h-80 overflow-y-auto py-2">
          ...
          {filtered.map((item) => (
            ...
            >
              ...
              <span className="ml-auto font-mono text-xs text-[var(--color-text-muted)]">{item.hint}</span>
            </button>
          ))}
        </div>
      </div>
